    ...
    with open(subject_path) as f:
        for line in f:
            if line.startswith("Expected files"):
                return [x.strip(',') for x in line.split(":", 1)[1].strip().split() if x]
    return []
...
def _show_solution():
    """Décode et écrit la solution du sujet courant dans traces/."""
    if current_subject is None:
        return
    level_name = LEVELS[current_level]
    ...
    # Headers associés (.h)
    headers = _SOLUTION_HEADERS.get(level_name, {}).get(subject_name, {})
    for filename, h_encoded in headers.items():
        dest = TRACES_DIR / filename
        dest.write_text(base64.b64decode(h_encoded).decode())
        print(f"Header écrit dans traces/{dest.name}")
...
def main():
    ...
    for i, arg in enumerate(sys.argv):
        if arg in _level_flags and i + 1 < len(sys.argv):
            parsed = _parse_levels(sys.argv[i + 1])
            LEVELS[:] = (parsed * 4)[:4]
            break
